...
In this post, I'm going to show how to monitor progress of download/upload using JSch library in Java. Let's see how step by step:

## 1. Establishing SSH connection

Before using SFTP, first we need to establish SSH connection to the remote host.
Following code demonstrates that how we can do this using JSch. (Of course, those codes don't have concern of following good practices. So, when you implement your code, please consider making your methods more modular, parametirezed etc.)
...
Let's trace what's happening by simplifying:
(You can think this as debugging.)
...
public class MySftpProgressMonitor implements SftpProgressMonitor {
    ...
    public boolean count(long count) {
        ...
        double progress = (double) this.totalBytes/totalBytes;
...
Okay, the above code is okay and functional but you may realized that printing in count() method is not the best solution because the count() method can be excessively called in get(...) method. So, the following implemantation would be a better:
...
public class MySftpProgressMonitor implements SftpProgressMonitor {
    ...
    public boolean count(long count) {
        this.totalBytes += count;
        this.progress = (double) this.totalBytes/max;
        return true;
    }
    ...
    public long getTransferredBytes(){
        return this.getTransferredBytes;
    }
